    ...
    return `
        <div class="sub-panel">
            <div>
                ...
                <div class="section-note">主看四项分数的加权距离，优先展示跨品牌对位。</div>
            </div>
            <div class="list-stack">
                ${
                    matches.length
...
function findNearestCards(targetGpu, limit) {
    const pool = DB.filter((gpu) => gpu.name !== targetGpu.name && gpu.segment === targetGpu.segment)
        .map((gpu) => ({
            ...
            overallDelta: computeOverallDelta(gpu, targetGpu),
        }))
...
function computeSimilarityDistance(base, candidate) {
    ...
    METRICS.forEach((metric) => {
        ...
        const candidateValue = candidate[metric.key];
        if (!isNumber(baseValue) || !isNumber(candidateValue) || Number(baseValue) === 0) return;

        const gap = Math.abs(((Number(candidateValue) - Number(baseValue)) / Number(baseValue)) * 100);
        weightedDistance += gap * metric.weight;
        weightSum += metric.weight;
    });

    return weightSum ? weightedDistance / weightSum : Infinity;
}
